...
### 1. **System Calls and fork()**

**Problem**: What happens when a multithreaded process calls `fork()`?

**Two Approaches**:

a) **Duplicate All Threads**
   - Fork creates new process with all threads duplicated
   - More resource intensive

b) **Duplicate Only Calling Thread**
   - Fork creates new process with only the thread that called `fork()`
   - Other threads are not duplicated
   - More common approach

**Usage**:
- If `exec()` follows `fork()`: Duplicate only calling thread (no point duplicating threads that will be replaced)
- If new process continues execution: Duplicate all threads

---

### 2. **Thread Cancellation**

**Definition**: Terminating a thread before it completes execution.
...
- Dangerous if thread was holding locks or updating shared data

#### b) Deferred Cancellation
- Thread checks periodically if it should terminate
- Terminates at safe points (**cancellation points**)
- Safer approach
- Thread can clean up resources before terminating

**Example Scenario**:
- User clicks "Cancel" button during long calculation
- Thread performing calculation should be cancelled
...
**Signal**: Notification to a process that an event has occurred (e.g., divide by zero, Ctrl+C)

**Problem**: In multithreaded program, which thread receives the signal?

**Options**:

1. **Deliver to the thread to which signal applies**
   - Synchronous signals (e.g., illegal memory access)
   - Delivered to thread that caused the signal

2. **Deliver to every thread in the process**
   - Asynchronous signals (e.g., Ctrl+C)

3. **Deliver to certain threads only**
   - Selective delivery based on signal masks

4. **Assign specific thread to handle all signals**
...
**Problem**: Continuously creating and destroying threads is expensive.

**Solution**: **Thread Pool**

#### What is a Thread Pool?

A collection of pre-created threads waiting to perform work.

#### How It Works:

1. **Initialization**: Create pool of threads at startup
2. **Request Arrival**: Task submitted to pool
3. **Assignment**: Idle thread picks up task and executes it
4. **Completion**: Thread returns to pool, waits for next task
5. **Reuse**: Same threads handle multiple tasks

#### Advantages

1. **Faster Service**: No thread creation overhead for each request
2. **Limited Threads**: Controls number of concurrent threads
3. **Resource Management**: Prevents system overload
4. **Separation of Concerns**: Task submission separated from execution

#### Example Use Cases
- Web servers handling client requests
- Database connection pools
- Worker thread pools in application servers

---

### 5. **Thread-Specific Data**

**Problem**: Sometimes threads need their own copy of certain data.

**Solution**: Thread-Local Storage (TLS)

**Thread-Specific Data**: Data unique to each thread, even though threads share most process data.

**Example**:
- Transaction ID in database threads
- User session data in web server threads
- Each thread needs its own copy

**Different from Local Variables**:
- Local variables exist only during function execution
- Thread-specific data persists across function calls within that thread
